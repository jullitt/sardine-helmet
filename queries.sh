...
echo "$($PSQL "
  ...
  FROM games g 
  INNER JOIN teams w ON g.winner_id = w.team_id 
  WHERE g.year = 2014 AND g.round = 'Eighth-Final'
  UNION ALL
  SELECT 
    o.name AS opponent_name 
  FROM games g 
  INNER JOIN teams o ON g.opponent_id = o.team_id 
  WHERE g.year = 2014 AND g.round = 'Eighth-Final' ORDER BY winner_name;")"
...
echo "$($PSQL "SELECT g.year AS year, t.name AS name FROM games g INNER JOIN teams t ON g.winner_id = t.team_id WHERE g.round = 'Final' ORDER BY year;")"
